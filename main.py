...
from independent_job.trainer import trainerTest
...
if __name__ == '__main__':
    ...
    if model_name == 'matrix':
        ...
        cfg.seed = SEED
        cfg.object = 'eng'
        cfg.resource = 'notsame(hard)'
        cfg.device = torch.device('cuda') if torch.cuda.is_available() else "cpu"
        cfg.model_params['device'] = cfg.device
        #_sac_edge_mean_pre_LN_SACA
        ...
        cfg.device = torch.device('cuda') if torch.cuda.is_available() else "cpu"
        cfg.model_params['device'] = cfg.device
        cfg.model_params['size'] = 'big' # big or orign

        # model_name/epoch/train_len/valid_len/job_len//seed 
        cfg.model_params['save_path'] = '{}_{}_{}_{}_{}_{}_{}_big.pth'.format(
                                                                cfg.object,
                                                                cfg.model_name,
                                                                cfg.epoch,
                                                                cfg.jobs_len,
                                                                SEED,
                                                                cfg.model_params['size'],
                                                                cfg.resource)    
