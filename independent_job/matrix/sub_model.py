...
class Depth_With_Edge_MultiHeadAttention(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        ...
        self.edge_out = nn.Linear(head_num * depth_hidden_dim, self.problem_size - 1)
    ...
    def forward(self, q, k, v, D_TM):
        ...
        qkv_dim = self.model_params['qkv_dim']
        sqrt_qkv_dim = self.model_params['sqrt_qkv_dim']
        depth_hidden_dim = self.model_params['depth_hidden_dim']
        dot_product = torch.matmul(q, k.transpose(2, 3))
        dot_product_score = dot_product / sqrt_qkv_dim
        ...
        ae_score_transposed = ae_score.transpose(2,3)
        # [B, T, M, H, depth_dim]
        ae_score_transposed_feature = ae_score_transposed.reshape(batch_size, T_cnt, M_cnt, head_num * depth_hidden_dim)
        # [B, T, M, H*depth_dim]
        edge_embedding = self.edge_out(ae_score_transposed_feature)
        edge_embedding_ac = F.gelu(edge_embedding)
        # [B, T, M, edge feature]
        
        fc_out = torch.matmul(ae_score, self.FC_weight) # a_e_feature로 ae_score 를  임시 대체 
        fc_out = fc_out + self.FC_bias[None, None, :, None, :]
        depth_scores = fc_out.transpose(1,2)
        # [B, H, T, M, 1]
        depth_scores = depth_scores.squeeze(-1)
        ...
        return out_concat, edge_embedding_ac
