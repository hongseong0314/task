...
class CloudMatrixModel(nn.Module):
    def __init__(self, **model_params):
        ...
        self.decoder = Matrix_Mix_Decoder(**model_params)
...
class CloudMatrixModel2(nn.Module):
    def __init__(self, **model_params):
        ...
        self.T_embedding = nn.Linear(self.nT, embedding_dim)
        self.M_embedding = nn.Linear(self.nM, embedding_dim)
        self.encoder = Matrix_Encoder2(**model_params)
        self.decoder = Matrix_Decoder(**model_params)
...
class Matrix_Encoder_one(nn.Module):
    def __init__(self, **model_params):
        ...
        self.layers = nn.ModuleList([EncoderLayer_one(**model_params) for _ in range(encoder_layer_num)])
...
class Matrix_Encoder2(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        encoder_layer_num = model_params['encoder_layer_num']
        self.layers = nn.ModuleList([EncoderLayer2(**model_params) for _ in range(encoder_layer_num)])
...
class EncoderLayer2(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        if model_params['TMHA'] == 'depth':
            self.row_encoding_block = EncodingBlock3(**model_params)
        else:
            self.row_encoding_block = EncodingBlock(**model_params)
    ...
    def forward(self, row_emb, col_emb, cost_mat):
        ...
        # col_emb.shape: (B, M, embedding)
        # cost_mat.shape: (B, T, M)
        row_emb_out, edge_row = self.row_encoding_block(row_emb, col_emb, cost_mat)
        col_emb_out, edge_col = self.row_encoding_block(col_emb, row_emb, cost_mat.transpose(1, 2))
        edge_feature = (edge_row + edge_col.transpose(1, 2)) / 2
        return row_emb_out, col_emb_out, edge_feature

class EncodingBlock3(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        embedding_dim = self.model_params['embedding_dim']
        ...
        self.mixed_score_MHA = Depth_With_Edge_MultiHeadAttention(**model_params)
        ...
        self.row_norm = nn.LayerNorm(embedding_dim)
        self.col_norm = nn.LayerNorm(embedding_dim)
    ...
    def forward(self, row_emb, col_emb, cost_mat):
        ...
        row_emb_norm = self.row_norm(row_emb)
        col_emb_norm = self.col_norm(col_emb)

        q = reshape_by_heads(self.Wq(row_emb_norm), head_num=head_num)
        # q shape: (batch, head_num, row_cnt, qkv_dim)
        k = reshape_by_heads(self.Wk(col_emb_norm), head_num=head_num)
        v = reshape_by_heads(self.Wv(col_emb_norm), head_num=head_num)
        # kv shape: (batch, head_num, col_cnt, qkv_dim)

        out_concat, edge_feature = self.mixed_score_MHA(q, k, v, cost_mat)
        ...
        out1_norm = self.FFN_norm(out1)
        # out1 = self.add_n_normalization_1(row_emb, multi_head_out)
        out2 = self.feed_forward(out1_norm)
        # out3 = self.add_n_normalization_2(out1, out2)
        out3 = out1 + out2
        # shape: (B, T, embedding)
        return out3, edge_feature
...
class Matrix_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.t_norm = nn.LayerNorm(embedding_dim)
    ...
    def forward(self, encoded_machine, encoded_jobs, ninf_mask):
        ...
        encoded_machine_norm = self.m_norm(encoded_machine)
        encoded_jobs_norm = self.t_norm(encoded_jobs)
        self.set_kv(encoded_jobs_norm)
        if self.skip:
        ...
        #######################################################
        q = reshape_by_heads(self.Wq_3(encoded_machine_norm), head_num=head_num)
        # shape: (B, H, M, qkv_dim)
...
class Matrix_Mix_Decoder(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        self.skip = self.model_params['skip']
